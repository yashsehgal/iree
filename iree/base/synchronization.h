...
// We have the CRITICAL_SECTION path for now but SRW is much better (and what
// std::mutex uses). SRW doesn't spin, though, and has some other implications
// that don't quite line up with pthread_mutex_t on most platforms. Once we have
// larger end-to-end benchmarks we should choose based on workloads.
