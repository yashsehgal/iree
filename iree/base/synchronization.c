...
// The SRW on Windows is pointer-sized and slightly better than what we emuate
...
void iree_slim_mutex_lock(iree_slim_mutex_t* mutex)
  ...
  // got wait for it to be available. Note that between the CAS above and this
...
// when tracing all slim mutices will be traced along with the fat mutices.
...
// The 64-bit value used to atomically RMW the state is split in two and treated
// as independent 32-bit ints:
//
//  MSB (63)                           32                               LSB (0)
// +-------------------------------------+-------------------------------------+
