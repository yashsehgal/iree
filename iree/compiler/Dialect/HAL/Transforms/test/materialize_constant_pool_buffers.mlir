...
hal.constant_pool @dense_variable_init attributes {buffer_constraints = #hal.buffer_constraints<max_allocation_size = 1073741824, min_buffer_offset_alignment = 32, max_buffer_range = 134217728, min_buffer_range_alignment = 4>} {
  // CHECK-NEXT: @cst0 {{.+}} -> @dense_variable_init_storage[#hal.byte_range<0, 512>]
  hal.constant_pool.span @cst0 : tensor<128xf32> = @_storage[#hal.byte_range<0, 512>]
  // CHECK-NEXT: @cst1 {{.+}} -> @dense_variable_init_storage[#hal.byte_range<512, 256>]
...
//      CHECK: hal.variable @dense_variable_init_storage init(@dense_variable_init_storage_initializer) : !hal.buffer
// CHECK-NEXT: func @dense_variable_init_storage_initializer() -> !hal.buffer
//      CHECK: [[STORAGE:%.+]] = hal.constant.storage @dense_variable_init::@_storage : !iree.byte_buffer
...
hal.constant_pool @pool attributes {buffer_constraints = #hal.buffer_constraints<max_allocation_size = 1073741824, min_buffer_offset_alignment = 32, max_buffer_range = 134217728, min_buffer_range_alignment = 4>} {
  // CHECK-NEXT: @cst0 {{.+}} -> @pool_storage0[#hal.byte_range<0, 16>]
  hal.constant_pool.span @cst0 : tensor<4xf32> = @_storage0[#hal.byte_range<0, 16>]
  // CHECK-NEXT: @cst1 {{.+}} -> @pool_storage1[#hal.byte_range<0, 3>]
...
//      CHECK: hal.variable @pool_storage0 init(@pool_storage0_initializer) : !hal.buffer
// CHECK-NEXT: func @pool_storage0_initializer() -> !hal.buffer
//      CHECK: [[STORAGE:%.+]] = hal.constant.storage @pool::@_storage0 : !iree.byte_buffer
//      CHECK: = hal.allocator.map {{.+}} [[STORAGE]][%c0, %c16] : !iree.byte_buffer -> !hal.buffer

//      CHECK: hal.variable @pool_storage1 init(@pool_storage1_initializer) : !hal.buffer
// CHECK-NEXT: func @pool_storage1_initializer() -> !hal.buffer
